...
const KIYO_DEFAULT = {
  ...
  STAGE_END: 'S',
  // 論文の進み具合を数えるための区切り。KIYO_STAGE_COLS で付けた名前のどれかを指定する。
  ARRIVED_STAGE: '入稿',       // ここまで進んでいなければ「未入稿」
...
function kiyoConfig_() {
  ...
  return {
    ...
    arrivedStage: toText_(getProp_(KIYO_PROP.ARRIVED_STAGE, KIYO_DEFAULT.ARRIVED_STAGE)),
...
function parseKiyoRows_(values, cfg, todayStr, bgs) {
  ...
  values.forEach(function (row, i) {
    ...
    // 入稿・組上がり・提出は、その工程以降まで進んでいれば済みとみなす（累計）。
    let doneLabel = '', last = null;
    const reached = { arrived: false, typeset: false, submitted: false };
    const idxOf = function (name) {
      let n = -1;
      cfg.stages.forEach(function (s, k) { if (s.name === name) n = k; });
      return n;
    };
    const marks = [
      { key: 'arrived', idx: idxOf(cfg.arrivedStage) },
      { key: 'typeset', idx: idxOf(cfg.typesetStage) },
      { key: 'submitted', idx: idxOf(cfg.submitStage) }
    ];
    const check = function (v, stageName, idx) {
      const t = toText_(v);
      if (!t) return;
      for (let k = 0; k < cfg.doneWords.length; k++) {
        // 校了・責了まで来ていれば、手前の工程はすべて済んでいるとみなす
        if (t.indexOf(cfg.doneWords[k]) >= 0) {
          doneLabel = cfg.doneWords[k];
          marks.forEach(function (m) { if (m.idx >= 0) reached[m.key] = true; });
          return;
        }
      }
      ...
      marks.forEach(function (m) { if (m.idx >= 0 && idx >= m.idx) reached[m.key] = true; });
    };
